...
export default function RepoCard({ repo, onDelete, onOpen }: Props) {
  ...
  return (
    <>
      <div
        className={`group relative rounded-2xl border bg-surface-card overflow-hidden flex flex-col transition-all duration-200
        ${cfg.border}
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                {(repo.status === "running" || repo.status === "pending") && (
                  <svg
                    className="w-4 h-4 text-yellow-400 shrink-0 animate-spin"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
              ...
              <p className="text-[11px] text-ink-subtle font-mono truncate mt-0.5">
                {repoShortName}
              </p>
            </div>
          </div>
          ...
          {repo.summary?.stack && repo.summary.stack.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {repo.summary.stack.slice(0, 4).map((tech) => (
                <span
                  key={tech}
                  className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[11px] font-semibold bg-surface-raised text-ink-muted border-surface-border"
                >
                  {techIconName(tech) ? (
          ...
          {repo.status === "failed" && (
            <div className="px-3 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 mt-auto">
              <p className="text-xs text-red-400">
                Indexing failed — delete and try again.
              </p>
            </div>
          )}
